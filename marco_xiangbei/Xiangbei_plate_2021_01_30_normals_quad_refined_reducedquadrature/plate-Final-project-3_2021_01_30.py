...
#from dolfinx.la import solve as solve_la
...
dx2 = dx(metadata={"quadrature_degree":3})
...
def energyPerUnitVolume_D(xi2):
    return 0.5*(dot(eps(xi2),D*eps(xi2)))

def energyPerUnitVolume_G(xi2):
    G = E/(2*(1+nu))
...
#    solve_la(A, target_func.vector, b)
...
elasticEnergy_D = energyPerUnitVolume_D*dxi2*dx2
elasticEnergy_G = energyPerUnitVolume_G*dxi2*dx
...
F = derivative(elasticEnergy_D+elasticEnergy_G,w,dw) \
    - inner(f,du_mid)*h*dx + Constant(mesh,1e-1)*E*h*dot(theta,n)*dot(dtheta,n)*dx  # !?!?!?!
...
#dolfinx.io.VTKFile("attempt.pvd").write(w_out)   
#File("u.pvd") << u_mid
#File("t.pvd") << theta
'''
