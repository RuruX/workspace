...
mesh = BoundaryMesh(BoxMesh(Point(0,0,0),Point(10,2,2),20,4,4),"exterior")

# Volume mesh generation gets expensive for large meshes, so, to get a
# more refined mesh, it is easier to just uniformly refine the boundary mesh.
# (However, this keeps the original faceted approximation to the geometry of
# any curved surfaces.)
N_REF = 2
...
# with x[0] negative.
bcs = [DirichletBC(W,Constant(6*(0,)),"x[0] < DOLFIN_EPS"),]
